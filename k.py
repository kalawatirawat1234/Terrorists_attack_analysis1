...
if not os.path.exists(shapefile_path):
    ...
    if 'geometry' in world.columns:
        merged = world.merge(country_attacks, left_on='geometry', right_on='country_txt', how='left')
        merged['attack_count'] = merged['attack_count'].fillna(0)

        fig, ax = plt.subplots(figsize=(10, 6))
        merged.plot(column='attack_count', cmap='OrRd', linewidth=0.8, ax=ax, edgecolor='0.8', legend=True)
        ax.set_title('Global Terrorist Attacks Heatmap')
        ax.axis('off')
        st.pyplot(fig)
    else:
        st.error("The 'name' column was not found in the shapefile. Please verify your shapefile structure.")
